...
contract MemeCoin is ERC20, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        maxHoldingPerWallet =
            (initialSupply * 1 ether * maxHoldingPercent) /
            HOLDING_PRECISION;
    ...
    function _update(
        ...
        if (
            (!taxEnabled || notTaxedFrom[from] || notTaxedTo[to]) &&
            !alwaysTaxedFrom[from] &&
            !alwaysTaxedTo[to]
        ) {
            if (
                !isMaxHoldingExempt[to] &&
                balanceOf(to) + amount > maxHoldingPerWallet
            ) revert MaxHoldingReached();
            ...
            if (
                !isMaxHoldingExempt[to] &&
                balanceOf(to) +
                    ((amount * (TAX_PRECISION - tax)) / TAX_PRECISION) >
                maxHoldingPerWallet
            ) revert MaxHoldingReached();
